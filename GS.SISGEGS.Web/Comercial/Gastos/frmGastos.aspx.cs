...
namespace GS.SISGEGS.Web.Comercial.Gastos
{
    ...
    {
        private void EgresosVarios_Listar(string idAgenda, DateTime fechaInicio, DateTime fechaFinal, string IdProveedor, string serie, decimal numero)
        {
            EgresosWCFClient objEgresosWCF;
            List<gsEgresosVarios_ListarCajaChicaResult> lstEgresosVarios;
            bool verPlanillas = false;
            try
            {
                objEgresosWCF = new EgresosWCFClient();
                if (((Usuario_LoginResult)Session["Usuario"]).aprobarPlanilla0 || ((Usuario_LoginResult)Session["Usuario"]).aprobarPlanilla1)
                    verPlanillas = true;

                lstEgresosVarios = objEgresosWCF.EgresosVarios_ListarCajaChica(((Usuario_LoginResult)Session["Usuario"]).idEmpresa,
                    ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, idAgenda, fechaInicio, fechaFinal, verPlanillas, IdProveedor, serie, numero ).ToList();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    EgresosVarios_Listar(((Usuario_LoginResult)Session["Usuario"]).nroDocumento, dpFechaInicio.SelectedDate.Value, dpFechaFinal.SelectedDate.Value, null,null,0);
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                   strNumero = Convert.ToDecimal(txtNumero.Text);
                }



                EgresosVarios_Listar(((Usuario_LoginResult)Session["Usuario"]).nroDocumento, dpFechaInicio.SelectedDate.Value, dpFechaFinal.SelectedDate.Value, Cliente, strSerie,strNumero);
        ...
        protected void grdEgresosVarios_DeleteCommand(object sender, GridCommandEventArgs e)
        {
            ...
            try
            {
                ...
                lstEgresosVarios = JsonHelper.JsonDeserialize<List<gsEgresosVarios_ListarCajaChicaResult>>((string)ViewState["lstEgresosVarios"]);
                lstEgresosVarios.Remove(lstEgresosVarios.Find(x => x.Op == Op));
                ViewState["lstEgresosVarios"] = JsonHelper.JsonSerializer(lstEgresosVarios);
                grdEgresosVarios.DataSource = lstEgresosVarios;
                grdEgresosVarios.DataBind();
            }
        ...
        protected void grdEgresosVarios_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            ...
            try
            {
                grdEgresosVarios.DataSource = JsonHelper.JsonDeserialize<List<gsEgresosVarios_ListarCajaChicaResult>>((string)ViewState["lstEgresosVarios"]);
            }
            catch (Exception ex)
            {
                lblMensaje.Text = "ERROR: " + ex.Message;
        ...
        private PdfPTable Add_Content_To_PDF(PdfPTable tableLayout, string fechaHasta, int idEmpresa, string Op)
        {
            ...
            lstEgresosVarios = JsonHelper.JsonDeserialize<List<gsEgresosVarios_ListarCajaChicaResult>>((string)ViewState["lstEgresosVarios"]);
            lstEgresosVarios.Find(x => x.Op == OpInt);
            EgresosVarios = lstEgresosVarios.Find(x => x.Op == OpInt);
