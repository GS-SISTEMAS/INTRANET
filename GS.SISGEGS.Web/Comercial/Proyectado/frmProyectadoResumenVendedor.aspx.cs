...
{
    public partial class frmProyectadoResumenVendedor : System.Web.UI.Page
    {
        ...
        private void Reporte_Cargar(int id_zonaV, string id_vendedorV)
        {
            ...
            List<gsPronostico_Reporte_VendedorResult> lstPronostico;
            try
            {
                ...
                lstPronostico = objReporteVenta.Pronostico_Reporte_Vendedor(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, fechaInicio, fechaFinal, id_zona , id_vendedor).ToList();
        ...
        protected void grdPronostico_NeedDataSource(object sender, PivotGridNeedDataSourceEventArgs e)
        {
            ...
            try
            {
                grdPronostico.DataSource = JsonHelper.JsonDeserialize<List<gsPronostico_Reporte_VendedorResult>>((string)ViewState["lstPronostico"]);
            }
            catch (Exception ex)
            {
                rwmReporte.RadAlert(ex.Message, 500, 100, "Error al descargar el excel", "");
