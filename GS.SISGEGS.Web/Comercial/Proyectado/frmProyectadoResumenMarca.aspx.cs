...
{
    public partial class frmProyectadoResumenMarca : System.Web.UI.Page
    {
        ...
        protected void grdPronostico_CellDataBound(object sender, Telerik.Web.UI.PivotGridCellDataBoundEventArgs e)
        {
        ...
        private void Reporte_Cargar(int id_zonaV, string id_vendedorV)
        {
            ...
            if(chkExpandir.Checked) grdPronostico.RowGroupsDefaultExpanded = true;
            else grdPronostico.RowGroupsDefaultExpanded = false;

            List<gsPronostico_Reporte_MarcaResult> lstPronostico;
            try
            {
                ...
                lstPronostico = objReporteVenta.Pronostico_Reporte_Marca(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, fechaInicio, fechaFinal, id_zona , id_vendedor).ToList();

                grdPronostico.DataSource = lstPronostico;
                grdPronostico.DataBind();

                lblMensaje.Text = "Se han encontrado " + lstPronostico.Count.ToString() + " registro.";
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                grdPronostico.ExportSettings.Excel.Format = (PivotGridExcelFormat)Enum.Parse(typeof(PivotGridExcelFormat), "Xlsx");
                grdPronostico.ExportSettings.FileName = "ReporteCostoMensual_" + DateTime.Now.ToString("yyyyMMddHmm");
                grdPronostico.ExportToExcel();
            }
            catch (Exception ex)
            {
                rwmReporte.RadAlert(ex.Message, 500, 100, "Error al descargar el excel", "");
        ...
        protected void grdPronostico_PivotGridCellExporting(object sender, PivotGridCellExportingArgs e)
        {
            try
            {
        ...
        protected void grdPronostico_NeedDataSource(object sender, PivotGridNeedDataSourceEventArgs e)
        {
            if (Session["Usuario"] == null)
                Response.Redirect("~/Security/frmCerrar.aspx");
            ...
            try
            {
                grdPronostico.DataSource = JsonHelper.JsonDeserialize<List<gsPronostico_Reporte_MarcaResult>>((string)ViewState["lstPronostico"]);
            }
            catch (Exception ex)
            {
                rwmReporte.RadAlert(ex.Message, 500, 100, "Error al descargar el excel", "");
