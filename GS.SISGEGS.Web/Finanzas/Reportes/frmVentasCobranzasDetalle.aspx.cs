...
{
    public partial class frmVentasCobranzasDetalle : System.Web.UI.Page
    {
        ...
        private decimal RatioMes( string ventaR , string cobranzaR)
        {
            decimal RatioDevuelto; 
            decimal ventasCV;
            decimal cobranzaCV;
            decimal ratioCV;
            ...
            numeroV = ventaR.Replace(" ","");
            numeroC = cobranzaR.Replace(" ", "");

            ventasCV = Convert.ToDecimal(numeroV);
            cobranzaCV = Convert.ToDecimal(numeroC);
            ...
            { ratioCV = cobranzaCV / ventasCV; }

            RatioDevuelto = ratioCV; 
            return RatioDevuelto; 
        }
        ...
        private void ListarVentasCobranzas(string codAgenda, string codVendedor, DateTime fechaInicial, DateTime fechaFinal,int year, int mesActual)
        {
            ...
            List<gsReporteCanceladosWebResumenResult> lstCobranzas = new List<gsReporteCanceladosWebResumenResult>();
            List<gsDocVenta_ReporteVenta_ClienteResumenResult> lstVentas = new List<gsDocVenta_ReporteVenta_ClienteResumenResult>(); 
            try
            {
                ...
                if (  codVendedor != null  )
                {
                    if (codVendedor.Length > 3)
                    {
                        if (codVendedor == "666666")
                        {
                            ...
                            List<Reporte_VentaxCobranzaLegalResult> Reporte = new List<Reporte_VentaxCobranzaLegalResult>();
                            List<Reporte_VentaxCobranzaLegalResult> lstVentaCobranzaR = objCobranzasWCF.Reporte_VentaCobranzaLegal(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario).ToList();

                            Reporte_VentaxCobranzaLegalResult RowVC1 = new Reporte_VentaxCobranzaLegalResult();
                            Reporte_VentaxCobranzaLegalResult RowVC2 = new Reporte_VentaxCobranzaLegalResult();
                            Reporte_VentaxCobranzaLegalResult RowVC3 = new Reporte_VentaxCobranzaLegalResult();
                            Reporte_VentaxCobranzaLegalResult RowVC4 = new Reporte_VentaxCobranzaLegalResult();
                            Reporte_VentaxCobranzaLegalResult RowVC5 = new Reporte_VentaxCobranzaLegalResult();
                            Reporte_VentaxCobranzaLegalResult RowVC6 = new Reporte_VentaxCobranzaLegalResult();
                            Reporte_VentaxCobranzaLegalResult RowVC7 = new Reporte_VentaxCobranzaLegalResult();

                            RowVC1 = lstVentaCobranzaR[0];
                            RowVC2 = lstVentaCobranzaR[1];
                            ...
                            RowVC3.conceptos = "Ratio (%) ";

                            ratioCV = RatioMes(RowVC1.enero, RowVC2.enero);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.enero = strRatioCV;

                            ratioCV = RatioMes(RowVC1.febrero, RowVC2.febrero);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.febrero = strRatioCV;

                            ratioCV = RatioMes(RowVC1.marzo, RowVC2.marzo);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.marzo = strRatioCV;

                            ratioCV = RatioMes(RowVC1.abril, RowVC2.abril);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.abril = strRatioCV;

                            ratioCV = RatioMes(RowVC1.mayo, RowVC2.mayo);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.mayo = strRatioCV;

                            ratioCV = RatioMes(RowVC1.junio, RowVC2.junio);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.junio = strRatioCV;

                            ratioCV = RatioMes(RowVC1.julio, RowVC2.julio);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.julio = strRatioCV;

                            ratioCV = RatioMes(RowVC1.agosto, RowVC2.agosto);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.agosto = strRatioCV;

                            ratioCV = RatioMes(RowVC1.septiembre, RowVC2.septiembre);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.septiembre = strRatioCV;

                            ratioCV = RatioMes(RowVC1.octubre, RowVC2.octubre);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.octubre = strRatioCV;

                            ratioCV = RatioMes(RowVC1.noviembre, RowVC2.noviembre);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.noviembre = strRatioCV;

                            ratioCV = RatioMes(RowVC1.diciembre, RowVC2.diciembre);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.diciembre = strRatioCV;

                            ratioCV = (TotalRatio/mesActual);
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.total = strRatioCV; 


                            RowVC4 = lstVentaCobranzaR[2];

                            TotalRatio = 0;
                            RowVC5.conceptos = "RatioMes (%) ";

                            ratioCV = RatioMes(RowVC1.enero, RowVC4.enero);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.enero = strRatioCV;

                            ratioCV = RatioMes(RowVC1.febrero, RowVC4.febrero);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.febrero = strRatioCV;

                            ratioCV = RatioMes(RowVC1.marzo, RowVC4.marzo);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.marzo = strRatioCV;

                            ratioCV = RatioMes(RowVC1.abril, RowVC4.abril);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.abril = strRatioCV;

                            ratioCV = RatioMes(RowVC1.mayo, RowVC4.mayo);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.mayo = strRatioCV;

                            ratioCV = RatioMes(RowVC1.junio, RowVC4.junio);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.junio = strRatioCV;

                            ratioCV = RatioMes(RowVC1.julio, RowVC4.julio);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.julio = strRatioCV;

                            ratioCV = RatioMes(RowVC1.agosto, RowVC4.agosto);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.agosto = strRatioCV;

                            ratioCV = RatioMes(RowVC1.septiembre, RowVC4.septiembre);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC3.septiembre = strRatioCV;

                            ratioCV = RatioMes(RowVC1.octubre, RowVC4.octubre);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.octubre = strRatioCV;

                            ratioCV = RatioMes(RowVC1.noviembre, RowVC4.noviembre);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.noviembre = strRatioCV;

                            ratioCV = RatioMes(RowVC1.diciembre, RowVC4.diciembre);
                            TotalRatio = TotalRatio + ratioCV;
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.diciembre = strRatioCV;

                            ratioCV = (TotalRatio / mesActual);
                            strRatioCV = ratioCV.ToString("P");
                            RowVC5.total = strRatioCV;

                            RowVC6 = lstVentaCobranzaR[3];
                            RowVC7 = lstVentaCobranzaR[4];
                            ...
                            dtTablaCruce = ConvertToDataTable(Reporte);
                        ...
                        {
                            List<gsReporteCanceladosWebResumenResult> lstCobranzasR = objCobranzasWCF.Reporte_CancelacionesResumen(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, codAgenda, codVendedor, year).ToList();
                            List<gsDocVenta_ReporteVenta_ClienteResumenResult> lstVentasR = objCobranzasWCF.Reporte_VentasClienteResumen(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, codAgenda, codVendedor, fechaInicial, fechaFinal, year).ToList();

                            var query_VentaA = from c in lstVentasR
                            ...
                            foreach (var Qventa in query_VentaA.ToList())
                            {
                                gsDocVenta_ReporteVenta_ClienteResumenResult lventa = new gsDocVenta_ReporteVenta_ClienteResumenResult();
                                lventa.Año = Qventa.Año;
                                lventa.ClaseCliente = Qventa.ClaseCliente;
                                lventa.ID_Cliente = Qventa.ID_Cliente;
                                lventa.Mes = Qventa.Mes;
                                lventa.Venta = Qventa.Venta;

                                lstVentas.Add(lventa);
                            }


                            var query_CobranzaA = from c in lstCobranzasR
                                                  where c.ClaseCliente != "LEGAL"
                                                  orderby c.periodoYear, c.periodoMes
                                                  select new
                                                  {
                                                      c.ClaseCliente,
                                                      c.Id_Cliente,
                            ...
                            foreach (var QCobranza in query_CobranzaA.ToList())
                            {
                                gsReporteCanceladosWebResumenResult lcobranza = new gsReporteCanceladosWebResumenResult();
                                lcobranza.periodoYear = QCobranza.periodoYear;
                                lcobranza.periodoMes = QCobranza.periodoMes;
                                lcobranza.ClaseCliente = QCobranza.ClaseCliente;
                                lcobranza.Id_Cliente = QCobranza.Id_Cliente;
                                lcobranza.totalMes = QCobranza.totalMes;

                                lstCobranzas.Add(lcobranza);
                            }
                        }
                    }
                ...
                else
                {
                    ...
                    lstCobranzas = lstCobranzasR;
                    lstVentas = lstVentasR;
                }
                ...
                if(LEGAL.Length == 0)
                {
                    ...
                    string strCobranza = "";
                    string strsVenta = "";
                    string strCobranzaTotal = "";
                    string strsVentaTotal = "";

                    DataRow dtRowV = dtTablaCruce.NewRow();
                    for (i = 0; i <= 14; i++)
                    {
                        if (i == 0)
                        {
                            dtRowV[i] = "Ventas " + year + " (IGV)";
                        }
                        if (i >= 1 & i <= 12)
                        {
                            var query_Venta = from c in lstVentas
                                              where c.Año == year & c.Mes == i
                                              orderby c.Año, c.Mes
                                              select new
                                              {
                            ...
                            if (query_Venta != null)
                            {
                                var sumImport = query_Venta.ToList().Select(c => c.Venta).Sum();
                                strsVenta = Convert.ToString(sumImport);
                                venta = Convert.ToDecimal(strsVenta);
                                strsVenta = string.Format("{0:#,##0.00}", sumImport);
                            }
                            else
                            {
                                venta = 0;
                        ...
                        if (i == 13)
                        {
                            strsVentaTotal = string.Format("{0:#,##0.00}", totalVenta);
                            dtRowV[i] = strsVentaTotal;
                    ...
                    DataRow dtRowC = dtTablaCruce.NewRow();
                    i = 0;
                    cobranza = 0;
                    for (i = 0; i <= 14; i++)
                    {
                        if (i == 0)
                        {
                            dtRowC[i] = "Cobranzas " + year;
                        }
                        if (i >= 1 & i <= 12)
                        {
                            var query_Cobranza = from c in lstCobranzas
                                                 where c.periodoYear == year & c.periodoMes == i
                                                 orderby c.periodoYear, c.periodoMes
                                                 select new
                                                 {
                                                     c.totalMes
                                                 };
                            if (query_Cobranza != null)
                            {
                                var sumImport = query_Cobranza.ToList().Select(c => c.totalMes).Sum();
                                strCobranza = Convert.ToString(sumImport);
                                cobranza = Convert.ToDecimal(strCobranza);
                                strCobranza = string.Format("{0:#,##0.00}", sumImport);
                            }
                            else
                            {
                                cobranza = 0;
                            ...
                            dtRowC[i] = strCobranza;
                        }
                        if (i == 13)
                        {
                            strCobranzaTotal = string.Format("{0:#,##0.00}", totalCobranza);
                            dtRowC[i] = strCobranzaTotal;
                        }
                        if (i == 14)
                        {
                            dtRowC[i] = codAgenda;
                    ...
                    dtTablaCruce.Rows.Add(dtRowV);
                    dtTablaCruce.Rows.Add(dtRowC);

                    rhcVentaCobranza.DataSource = GetData(dtTablaCruce);
                    rhcVentaCobranza.DataBind();


                    DataRow dtRowP = dtTablaCruce.NewRow();
                    i = 0;
                    decimal ratio = 0;
                    decimal ratioPromedio = 0;
                    decimal ventasR = 0;
                    decimal cobranzaR = 0;
                    string strRatio;
                    string strRatioPromedio;
                    ...
                    for (i = 0; i <= 14; i++)
                    {
                        ...
                        if (i >= 1 & i <= 12)
                        {
                            string str = dtTablaCruce.Rows[0][i].ToString();
                            ventasR = Convert.ToDecimal(str);

                            cobranzaR = Convert.ToDecimal(dtTablaCruce.Rows[1][i]);
                            ...
                            {
                                ratio = cobranzaR / ventasR;
                            }


                            strRatio = ratio.ToString("P");
                        ...
                        if (i == 13)
                        {
                            ...
                            {
                                ratioPromedio = (totalCobranza / totalVenta);
                            }

                            strRatioPromedio = ratioPromedio.ToString("P");
                            dtRowP[i] = strRatioPromedio;
                        }
                        if (i == 14)
                        {
                            dtRowP[i] = codAgenda;
                    ...
                    dtTablaCruce.Rows.Add(dtRowP);

                    //-----------------
                    DataRow dtRowB = dtTablaCruce.NewRow();
                    i = 0;
                    ...
                    dtTablaCruce.Rows.Add(dtRowB);

                    //-----------------
                    DataRow dtRowCA = dtTablaCruce.NewRow();
                    i = 0;
                    decimal cobrado = 0;
                    decimal TotalCobrado = 0;
                    string strcobrado = "";
                    string strtotalcobrado = "";
                    ...
                    for (i = 0; i <= 14; i++)
                    {
                        if (i == 0)
                        {
                            dtRowCA[i] = "CobradoMes";
                        }
                        if (i >= 1 & i <= 12)
                        {
                            if (i <= mesActual)
                            {
                                cobrado = CobradoMesActual(codAgenda, codVendedor, year, i);
                                strcobrado = string.Format("{0: #,##0.00}", cobrado);
                            }
                            else
                            {
                                cobrado = 0;
                            ...
                            dtRowCA[i] = strcobrado;
                        }
                        if (i == 13)
                        {
                            TotalCobrado = (TotalCobrado / mesActual);
                            strtotalcobrado = string.Format("{0: #,##0.00}", TotalCobrado);
                            dtRowCA[i] = strtotalcobrado;
                        }
                        if (i == 14)
                        {
                            dtRowCA[i] = codAgenda;
                    ...
                    dtTablaCruce.Rows.Add(dtRowCA);
                    //----------------------------------

                    DataRow dtRowPM = dtTablaCruce.NewRow();
                    i = 0;
                    decimal ratioM = 0;
                    decimal ratioPromedioM = 0;
                    decimal ventasRM = 0;
                    decimal cobranzaRM = 0;
                    string strRatioM;
                    string strRatioPromedioM;
                    ...
                    for (i = 0; i <= 14; i++)
                    {
                        ...
                        if (i >= 1 & i <= 12)
                        {
                            ventasRM = Convert.ToDecimal(dtTablaCruce.Rows[0][i]);
                            string numero; 
                            numero = Convert.ToString(dtTablaCruce.Rows[4][i]);
                            numero = numero.Replace(" ", ""); 

                            cobranzaRM = Convert.ToDecimal(numero);
                            ...
                            {
                                ratioM = cobranzaRM / ventasRM;
                            }

                            strRatioM = ratioM.ToString("P");
                        ...
                        if (i == 13)
                        {
                            ...
                            {
                                ratioPromedioM = (TotalCobrado / totalVenta);
                            }

                            strRatioPromedioM = ratioPromedioM.ToString("P");
                            dtRowPM[i] = strRatioPromedioM;
                        }
                        if (i == 14)
                        {
                            dtRowPM[i] = codAgenda;
                    ...
                    dtTablaCruce.Rows.Add(dtRowPM);

                    //----------------------------------
                    DataRow dtRowD = dtTablaCruce.NewRow();
                    //DataRow dtRowDA = dtTablaCruce.NewRow();

                    i = 0;
                    decimal Vencidos = 0;
                    decimal totalVencidos = 0;
                    string strVencidos = "";
                    string strtotalVencidos = "";
                    ...
                    for (i = 0; i <= 14; i++)
                    {
                        ...
                        if (i >= 1 & i <= 12)
                        {
                            if (i <= mesActual)
                            {
                                //Vencidos = DeudaVencidaMes(codAgenda, codVendedor, year, i);
                                Vencidos = DeudaVencidaMesBI(codAgenda, codVendedor, year, i);
                                strVencidos = string.Format("{0: #,##0.00}", Vencidos);
                            ...
                            totalVencidos = totalVencidos + Vencidos;
                            dtRowD[i] = strVencidos;
                        }
                        if (i == 13)
                        {
                            totalVencidos = (totalVencidos / mesActual);
                            strtotalVencidos = string.Format("{0: #,##0.00}", totalVencidos);
                            dtRowD[i] = strtotalVencidos;
            ...
            for (x = 1; x < column - 2 ; x++)
              {
                ...
                for (y = 0; y < rows; y++)
                {
                   ...
                   if (y == 0 )
                    {
                            strNumero = dtTabla.Rows[y][x].ToString();
        ...
        public decimal DeudaVencidaMes(string codAgenda, string codVendedor, int anho, int mes)
        {
            ...
            try
            {
                List<gsReporteVencidosPorMesDetalleResult> lstPendientes = BuscarDocumentosPendientes_VencidosMes(codAgenda, codVendedor,  fecha);
        ...
        public decimal DeudaVencidaMesBI(string codAgenda, string codVendedor, int anho, int mes)
        {
            decimal deudaVencidaMes = 0;
            DateTime fecha;
            ...
            try
            {
                List<gsEstadoCuenta_MesBIResult> lstPendientes = BuscarDocumentosPendientes_VencidosMesBI(codAgenda, codVendedor, fecha);
                ViewState["lstEstadoCuentaAfiliadas"] = JsonHelper.JsonSerializer(lstPendientes);

                if(codAgenda != null || codVendedor != null )
                {
                    var query_Detalle = from c in lstPendientes
        ...
        public decimal CobradoMesActual(string codAgenda, string codVendedor, int anho, int mes)
        {
            ...
            CobranzasWCFClient objCobranzasWCF = new CobranzasWCFClient();
            List<gsReporteCanceladosResumenMes_v3Result> lstCobranzas = new List<gsReporteCanceladosResumenMes_v3Result>();
            try
            {
         
                
                lstCobranzas = objCobranzasWCF.Reporte_CancelacionesResumenActual_v3(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, codAgenda, codVendedor, anho, mes).ToList();

                if(codVendedor != null || codAgenda != null)
                {
                ...
                {
                    var query_Detalle = from c in lstCobranzas
                                        where c.periodoYearE == anho & c.periodoMesE == mes & c.ClaseCliente == "LEGAL"
                                        select new
                                        {
                                            c.periodoYearE,
                                            c.periodoMesE,
        ...
        public List<gsReporteVencidosPorMesDetalleResult> BuscarDocumentosPendientes_VencidosMes(string id_cliente, string id_vendedor, DateTime fechaForm2)
        {
            List<gsReporteVencidosPorMesDetalleResult> lst = new List<gsReporteVencidosPorMesDetalleResult>();
            ...
            try
            {
                if (Validar_Variables() == 0)
                {
                    lst = ListarEstadoCuenta_VencidoMes(id_cliente, id_vendedor, fechaForm2);
                }
        ...
        public List<gsEstadoCuenta_MesBIResult> BuscarDocumentosPendientes_VencidosMesBI(string id_cliente, string id_vendedor, DateTime fechaForm2)
        {
            List<gsEstadoCuenta_MesBIResult> lst = new List<gsEstadoCuenta_MesBIResult>();
            ...
            try
            {
                if (Validar_Variables() == 0)
                {
                    lst = ListarEstadoCuenta_VencidoMesBI(id_cliente, id_vendedor, fechaForm2);
                }
        ...
        private List<gsReporteVencidosPorMesDetalleResult> ListarEstadoCuenta_VencidoMes( string id_cliente, string id_vendedor,  DateTime fechaEmisionFinal)
        {
            EstadoCuentaWCFClient objEstadoCuentaWCF = new EstadoCuentaWCFClient();
            try
            {
                List<gsReporteVencidosPorMesDetalleResult> lst = objEstadoCuentaWCF.EstadoCuenta_VencidosMesDetalle(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, id_cliente, id_vendedor,  fechaEmisionFinal).ToList();
        ...
        private List<gsEstadoCuenta_MesBIResult> ListarEstadoCuenta_VencidoMesBI(string id_cliente, string id_vendedor, DateTime fechaEmisionFinal)
        {
            EstadoCuentaWCFClient objEstadoCuentaWCF = new EstadoCuentaWCFClient();
            try
            {
                List<gsEstadoCuenta_MesBIResult> lst = objEstadoCuentaWCF.EstadoCuenta_ListarxBI(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, id_cliente, id_vendedor, fechaEmisionFinal).ToList();
        ...
        protected void grdVentasCobranzas_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            if(colum == 2 || colum == 10 || colum == 14 || colum == 16 )
            {
