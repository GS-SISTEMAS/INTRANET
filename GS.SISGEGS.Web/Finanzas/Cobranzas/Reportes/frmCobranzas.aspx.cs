...
{
    public partial class frmCobranzas : System.Web.UI.Page
    {
        private List<gsReporteCanceladosWebResult> ListarCobranzas(string codAgenda, string codVendedor,  DateTime fechaInicial, DateTime fechaFinal)
        {
            CobranzasWCFClient objCobranzasWCF = new CobranzasWCFClient();
            try
            {
                List<gsReporteCanceladosWebResult> lstCobranza = new List<gsReporteCanceladosWebResult>(); 
                ...
                if (codVendedor != null)
                {
                    if (codVendedor.Length > 3)
                    {
                        if (codVendedor == "666666")
                        {
                            ...
                            List<gsReporteCanceladosWebResult> lst = objCobranzasWCF.Reporte_Cancelaciones(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, codAgenda, codVendedor, fechaInicial, fechaFinal).ToList();

                            var query_Cobranza = from c in lst
                                               where c.EstadoCliente == "LEGAL"
                                               orderby c.Cliente, c.DiasMora descending
                                               select new
                                               {
                                                   c.Banco,
                                                   c.Cliente,
                                                   ...
                                                   c.NumeroDocumento,
                                                   c.PagadoDolares,
                                                   c.TC, c.TipoDoc, c.Vendedor
                                                
                                               };

                            foreach (var QCobranza in query_Cobranza.ToList())
                            {
                                gsReporteCanceladosWebResult rowCobranza = new gsReporteCanceladosWebResult();
                                rowCobranza.Banco = QCobranza.Banco;
                                rowCobranza.Cliente = QCobranza.Cliente;
                                rowCobranza.CtaCte = QCobranza.CtaCte;
                                rowCobranza.DiasMora = QCobranza.DiasMora;
                                rowCobranza.EstadoCliente = QCobranza.EstadoCliente;
                                rowCobranza.FechaAplicado = QCobranza.FechaAplicado;
                                rowCobranza.FechaCobranza = QCobranza.FechaCobranza;
                                rowCobranza.FechaEmision = QCobranza.FechaEmision;
                                rowCobranza.FechaVencimiento = QCobranza.FechaVencimiento;
                                rowCobranza.Glosa = QCobranza.Glosa;
                                rowCobranza.ID_Banco = QCobranza.ID_Banco;
                                rowCobranza.Id_Cliente = QCobranza.Id_Cliente;
                                rowCobranza.Id_Moneda = QCobranza.Id_Moneda;
                                rowCobranza.Id_TipoDoc = QCobranza.Id_TipoDoc;
                                rowCobranza.Id_TipoPago = QCobranza.Id_TipoPago;
                                rowCobranza.ID_Vendedor = QCobranza.ID_Vendedor;
                                rowCobranza.ImporteCancelado = QCobranza.ImporteCancelado;
                                rowCobranza.Mon = QCobranza.Mon;
                                rowCobranza.Moneda = QCobranza.Moneda;
                                rowCobranza.NombreTipoPago = QCobranza.NombreTipoPago;

                                rowCobranza.NroCuenta = QCobranza.NroCuenta;
                                rowCobranza.NumeroDocumento = QCobranza.NumeroDocumento;
                                rowCobranza.PagadoDolares = QCobranza.PagadoDolares;
                                rowCobranza.TC = QCobranza.TC;
                                rowCobranza.TipoDoc = QCobranza.TipoDoc;
                                rowCobranza.Vendedor = QCobranza.Vendedor;

                                lstCobranza.Add(rowCobranza);
                            }
                        }
                        else
                        {
                            ...
                            var query_Cobranza = from c in lst
                                                 where c.EstadoCliente != "LEGAL" & c.EstadoCliente != "AFILIADA"
                                                 orderby c.Cliente, c.DiasMora descending
                                                 select new
                                                 {
                                                     c.Banco,
                ...
                ViewState["lstCobranzas"] = JsonHelper.JsonSerializer(lstCobranza);
                grdCobranzas.DataSource = lstCobranza;
                grdCobranzas.DataBind();
                lblDate.Text = "1";
                return lstCobranza;
            }
            catch (Exception ex)
        ...
        private List<gsReporteCanceladosWebResumenVendedorResult> ListarCobranzasResumenVendedor(string codAgenda, string codVendedor, DateTime fechaInicial, DateTime fechaFinal)
        {
            CobranzasWCFClient objCobranzasWCF = new CobranzasWCFClient();
            try
            {
                List<gsReporteCanceladosWebResumenVendedorResult> lst = objCobranzasWCF.Reporte_CancelacionesResumenVendedor(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, codAgenda, codVendedor, fechaInicial, fechaFinal).ToList();

                ViewState["lstCobranzasResumenVendedor"] = JsonHelper.JsonSerializer(lst);

                grdCobranzasResumenPivot.DataSource = lst;
                grdCobranzasResumenPivot.DataBind();
                lblDate2.Text = "2";
                return lst;
            }
            catch (Exception ex)
        ...
        protected void grdCobranzasResumenPivot_NeedDataSource(object sender, PivotGridNeedDataSourceEventArgs e)
        {
            ...
            try
            {
                if (lblDate2.Text == "2")
                {
                    grdCobranzasResumenPivot.DataSource = JsonHelper.JsonDeserialize<List<gsReporteCanceladosWebResumenVendedorResult>>((string)ViewState["lstCobranzasResumenVendedor"]);
                }
        ...
        protected void grdCobranzasResumenPivot_PivotGridCellExporting(object sender, PivotGridCellExportingArgs e)
        {
            try
            {
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                grdCobranzasResumenPivot.ExportSettings.Excel.Format = (PivotGridExcelFormat)Enum.Parse(typeof(PivotGridExcelFormat), "Xlsx");
                grdCobranzasResumenPivot.ExportSettings.FileName = "ReporteCostoMensual_" + DateTime.Now.ToString("yyyyMMddHmm");
                grdCobranzasResumenPivot.ExportToExcel();
            }
            catch (Exception ex)
            {
        ...
        private List<gsReporte_DeudaVencidaTotalResult> ListarEstadoCuentaResumenTotal(string codAgenda, string codVendedor, DateTime fechaEmisionInicial, DateTime fechaEmisionFinal, DateTime fechaVencimientoInicial, DateTime fechaVencimientoFinal, int vencidos)
        {

            EstadoCuentaWCFClient objEstadoCuentaWCF = new EstadoCuentaWCFClient();
            try
            {
                List<gsReporte_DeudaVencidaTotalResult> lst = objEstadoCuentaWCF.EstadoCuenta_ListarResumenTotal(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, codAgenda, codVendedor, fechaEmisionInicial, fechaEmisionFinal, fechaVencimientoInicial, fechaVencimientoFinal, vencidos).ToList();
                ...
                //ViewState["lstEstadoCuentaResumenVencidos"] = JsonHelper.JsonSerializer(query_Detalle);
                grdVencidosResumen.DataSource = query_Detalle;
                grdVencidosResumen.DataBind();
                lblDate.Text = "1";
                return lst;
            }
            catch (Exception ex)
        ...
        protected void BuscarVencidosTotal(DateTime fechaHasta, string cliente, string vendedor)
        {
            DateTime fecha1;
            DateTime fecha2;
            DateTime fecha3;
            ...
            List<gsReporte_DeudaVencidaTotalResult> lst;

            lblMesajeResumen.Text = "";
            if (Session["Usuario"] == null)
                Response.Redirect("~/Security/frmCerrar.aspx");
            ...
            try
            {
                ...
                vencidos = 0;
                 ...
                 lst = ListarEstadoCuentaResumenTotal(Cliente1, Vendedor1, fecha1, fecha2, fecha3, fecha4, vencidos);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            scriptManager.RegisterPostBackControl(this.btnExpDetalle);
            ...
            try
            {
                if (!Page.IsPostBack) {
                    ...
                    dpDesde.SelectedDate = DateTime.Now.AddDays(- DateTime.Now.Day + 1 );
                    dpHasta.SelectedDate = DateTime.Now;

                    dpFechaDesdeResumen.SelectedDate = DateTime.Now.AddDays(-DateTime.Now.Day + 1);
                    dpFechaHastaResumen.SelectedDate = DateTime.Now;

                    lblDate.Text = "";
                    lblDate2.Text = "2";

                    //ReporteResumen_Cargar();
                    List<gsReporteCanceladosWebResumenVendedorResult> lst = new List<gsReporteCanceladosWebResumenVendedorResult>(); 
                    ViewState["lstCobranzasResumenVendedor"] = JsonHelper.JsonSerializer(lst);
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            Cliente = "";
            Vendedor = "";
            List<gsReporteCanceladosWebResult> lst;
            ...
            try
            {
                ...
                {
                    fecha1 = dpDesde.SelectedDate.Value;
                    fecha2 = dpHasta.SelectedDate.Value;
                    ...
                    lst = ListarCobranzas(Cliente, Vendedor, fecha1 , fecha2 );
                }
        ...
        public int Validar_Variables()
        {
            ...
            if ( dpDesde == null || dpDesde.SelectedDate.Value.ToString() == "")
            {
                valor = 1;
                lblMensaje.Text = lblMensaje.Text + "Seleccionar fecha desde. ";
                lblMensaje.CssClass = "mensajeError";
            }
            if (dpHasta == null || dpHasta.SelectedDate.Value.ToString() == "")
            {
                valor = 1;
                lblMensaje.Text = lblMensaje.Text + "Seleccionar fecha hasta. ";
                lblMensaje.CssClass = "mensajeError";
            }
        ...
        protected void grdCobranzas_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            try
            {
                if (lblDate.Text == "1")
        ...
        protected void btnExpDetalle_Click(object sender, ImageClickEventArgs e)
        {
            ...
            try
            {
                Response.AppendHeader("content-disposition", "attachment;filename=ReporteDetallado.xls");
                Response.Charset = "";
                Response.Cache.SetCacheability(HttpCacheability.NoCache);
                Response.ContentType = "application/vnd.ms-excel";
                this.EnableViewState = false;
                Response.Write("<html><head></head><body>"+ ExportGridDetalleToHTML() + "</body></html>");
                Response.End();
            }
            catch (Exception ex)
            {
        ...
        string ExportGridDetalleToHTML() {

            string strHTML = "<table>";
            strHTML += "<tr>";
            strHTML += "<td>Fecha Cobranza:</td>";
            strHTML += "<td>"+ ((DateTime)dpDesde.SelectedDate).ToShortDateString() + " - " + ((DateTime)dpHasta.SelectedDate).ToShortDateString() +"</td>";
            strHTML += "</tr>";
            strHTML += "<tr>";
            strHTML += "<td>Cliente:</td>";
            strHTML += "<td>"+acbCliente.Text+"</td>";
            strHTML += "</tr>";
            strHTML += "<tr>";
            strHTML += "<td>Vendedor:</td>";
            strHTML += "<td>"+acbVendedor.Text+"</td>";
            strHTML += "</tr>";
            strHTML += "</table>";

            strHTML += "<table><tr><td></td></tr></table>";

            strHTML += "<table border=1>";
            strHTML += "<tr>";
            strHTML += "<td><b>CODIGO</b></td>";
            strHTML += "<td><b>CLIENTE</b></td>";
            strHTML += "<td><b>TIPO DOCUMENTO</b></td>";
            strHTML += "<td><b>NUM DOCUMENTO</b></td>";
            strHTML += "<td><b>FECHA EMISION</b></td>";
            strHTML += "<td><b>FECHA VENCIMIENTO</b></td>";
            strHTML += "<td><b>FECHA COBRANZA</b></td>";
            strHTML += "<td><b>DIAS MORA</b></td>";
            strHTML += "<td><b>MONEDA</b></td>";
            strHTML += "<td><b>IMPORTE PAGO</b></td>";
            strHTML += "<td><b>TC</b></td>";
            strHTML += "<td><b>PAGO DOLARES</b></td>";
            strHTML += "<td><b>BANCO</b></td>";
            strHTML += "<td><b>CtaCte</b></td>";
            strHTML += "<td><b>Nro Cuenta</b></td>";
            strHTML += "</tr>";
            List<gsReporteCanceladosWebResult> lst = JsonHelper.JsonDeserialize<List<gsReporteCanceladosWebResult>>((string)ViewState["lstCobranzas"]);
            foreach (var item in lst) {
                strHTML += "<tr>";
                strHTML += "<td>"+item.Id_Cliente+"</td>";
                strHTML += "<td>"+item.Cliente+"</td>";
                strHTML += "<td>"+item.TipoDoc+"</td>";
                strHTML += "<td>"+item.NumeroDocumento+"</td>";
                strHTML += "<td>"+item.FechaEmision.ToShortDateString()+"</td>";
                strHTML += "<td>"+item.FechaVencimiento.ToShortDateString() + "</td>";
                strHTML += "<td>"+item.FechaCobranza.ToShortDateString() + "</td>";
                strHTML += "<td>"+item.DiasMora+"</td>";
                strHTML += "<td>"+item.Mon+"</td>";
                strHTML += "<td>"+item.ImporteCancelado+"</td>";
                strHTML += "<td>"+item.TC+"</td>";
                strHTML += "<td>"+item.PagadoDolares+"</td>";
                strHTML += "<td>"+item.Banco+"</td>";
                strHTML += "<td>"+item.CtaCte+"</td>";
                strHTML += "<td>'"+item.NroCuenta.ToString()+"</td>";
                strHTML += "</tr>";
            }
        ...
        //    fechaHasta = dpHasta.SelectedDate.Value.ToString("dd/MM/yyyy");
        ...
        protected void btnBuscarResumen_Click(object sender, EventArgs e)
        {
            lblMensaje.Text = "";
            if (Session["Usuario"] == null)
                Response.Redirect("~/Security/frmCerrar.aspx");
        ...
        private void ReporteResumen_Cargar()
        {
            try
            {
                DateTime fecha1;
                ...
                Cliente = "";
                Vendedor = "";
                List<gsReporteCanceladosWebResumenVendedorResult> lst;
                ...
                try
                {
                    fecha1 = dpFechaDesdeResumen.SelectedDate.Value;
                    fecha2 = dpFechaHastaResumen.SelectedDate.Value;

                    Cliente = null;
                    Vendedor = null;

                    lst = ListarCobranzasResumenVendedor(Cliente, Vendedor, fecha1, fecha2);

                    BuscarVencidosTotal(fecha2, Cliente, Vendedor);
            ...
            {
                lblMesajeResumen.Text = "No se encontrarón resultados.";
                lblMesajeResumen.CssClass = "mensajeError";
            }
        }
