    ...
    public partial class frmFacturaElectronica_Usu : System.Web.UI.Page
    {
        ...
        void RegistrarDarBajaComprobante(string ruc, string numero, string serie)
        {
            ...
            try
            {
                ...
                bool ExitoR; 
                Exito = oServicioOK2.RegistrarComunicacionBaja(oGeneral, cadena, ref oRespuestaError,0);
                ExitoR = Exito; 
            }
            catch (Exception ex)
            {
        ...
        void RegistrarResumenDiario(string ruc, string numero, string serie)
        {
            ...
            try
            {
                ...
                Exito = oServicioOK2.RegistrarResumen(oResumen,ref cadena, 0);
                ExitoR = Exito;
            }
            catch (Exception ex)
            {
        ...
        protected void btnRegistrar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                foreach (GridItem rowitem in grdFacturaElectronica.MasterTableView.Items)
                {
                    ...
                    TableCell cell = dataitem["CheckColumn"];
                    CheckBox checkBox = (CheckBox)cell.Controls[0].FindControl("Check");
                    objComprobante = new FacturacionElectronicaOkWCF.ENComprobante();
                    oGeneral = new FacturacionElectronicaOkWCF.General();
                    ...
                    if (checkBox.Checked)
                    {
                        ...
                        {
                            EnviarFacturaElectronica(Op, TablaOrigen, Serie, Numero, Otorgamiento);
                        }
                        else if (DocSunat == 3) // Boleta (Venta)
                        {
                            EnviarBoletaElectronica(Op, TablaOrigen, Serie, Numero, Otorgamiento);
                        }
                        else if (DocSunat == 8) // Nota Debito
                        {
                            ...
                            EnviarNotaDebitoElectronica(Op, TablaOrigen, Serie, Numero, ReferenciaDocumentoTipo, ReferenciaDocumentoNombre, ReferenciaDocumentoSerie, ReferenciaDocumentoNumero, ReferenciaDocumentoFecha, Otorgamiento);
                        }
                        else if (DocSunat == 7) // Nota Credito (Venta)
                        {
                            ...
                            EnviarNotaCreditoElectronica(Op, TablaOrigen, Serie, Numero, ReferenciaDocumentoTipo, ReferenciaDocumentoNombre, ReferenciaDocumentoSerie, ReferenciaDocumentoNumero, ReferenciaDocumentoFecha, Otorgamiento);
        ...
        protected void btnRegistrarPrueba_Click(object sender, EventArgs e)
        {
            int intSolo = 0;
        ...
        void EnviarFacturaElectronica(int Op, string TablaOrigen, string Serie, string Numero, string Otorgamiento)
        {
            try
            {
                string SerieF ="";
                ...
                LlenarEmpresa(ref NroCuenta); //Lleno Datos de la Empresa Emisora
                LlenarComprobanteFactura(Op, TablaOrigen, SerieF, Numero, ref Archivo, NroCuenta); //Lleno los Datos del Comprobante
                ...
                {
                    result = oServicio.Registrar(oGeneral, FacturacionElectronicaOkWCF.TipoDocumento.Factura, ref cadena, tipoCodigo, ref codigoBarra, ref codigoHash, ref ListaError, 0, Otorgamiento);
                }
                else if (objComprobante.TipoComprobante == "03")
                {
                    result = oServicio.Registrar(oGeneral, FacturacionElectronicaOkWCF.TipoDocumento.Boleta, ref cadena, tipoCodigo, ref codigoBarra, ref codigoHash, ref ListaError, 0, Otorgamiento);
                }
                else if (objComprobante.TipoComprobante == "07")
                {
                    result = oServicio.Registrar(oGeneral, FacturacionElectronicaOkWCF.TipoDocumento.NotaCredito, ref cadena, tipoCodigo, ref codigoBarra, ref codigoHash, ref ListaError, 0, Otorgamiento);
                }
                else if (objComprobante.TipoComprobante == "08")
                {
                    result = oServicio.Registrar(oGeneral, FacturacionElectronicaOkWCF.TipoDocumento.NotaDebito, ref cadena, tipoCodigo, ref codigoBarra, ref codigoHash, ref ListaError, 0, Otorgamiento);
                }
        ...
        void EnviarBoletaElectronica(int Op, string TablaOrigen, string Serie, string Numero, string Otorgamiento)
        {
            try
            {
                string SerieF = "";
        ...
        void EnviarNotaCreditoElectronica(int Op, string TablaOrigen, string Serie, string Numero, string ReferenciaDocumentoTipo, string ReferenciaDocumentoNombre, string ReferenciaDocumentoSerie, string ReferenciaDocumentoNumero, string ReferenciaDocumentoFecha, string Otorgamiento)
        {
            string SerieNC = "";
            string SerieRef = "";
        ...
        void EnviarNotaDebitoElectronica(int Op, string TablaOrigen, string Serie, string Numero, string ReferenciaDocumentoTipo,string ReferenciaDocumentoNombre, string ReferenciaDocumentoSerie, string ReferenciaDocumentoNumero, string ReferenciaDocumentoFecha, string Otorgamiento)
        {
            string SerieND = "";
            string SerieRef = "";
        ...
        void LlenarComprobanteFactura(int Op, string TablaOrigen, string serie, string numero, ref string Archivo, string NroCuenta)
        {
            string CASO = "";
            int intSolo = 0; 
            ...
            try
            {
                ...
                objComprobante.CorreoElectronico = objFacturaCabecera.Cliente_Correo.ToString().Trim(); //"jean.dedios@gruposilvestre.com.pe";
                ...
                if (objFacturaCabecera.TREFI == 0)
                {
                    Texto.Texto3 = "Los derechos e importes contenidos en este documento han sido cedidos de manera irrevocable a un patrimonio autónomo administrado por Scotiabank Perú S.A.A.";
                }

                if (objFacturaCabecera.TREFI == 99)
                {
                    var IdEmpresa = ((Usuario_LoginResult)HttpContext.Current.Session["Usuario"]).idEmpresa;
                    var TextoB = "Los importes contenidos en el presente comprobante de pago han sido transferidos a un patrimonio fideicometido administrado por La Fiduciaria S.A. En consecuencia, los pagos debidos de acuerdo a este comprobante de pago deberán realizarse únicamente a la siguiente cuenta bancaria, abierta en: ";

                    if (IdEmpresa == 1)
                    {
                        if (objFacturaCabecera.TipoMoneda == "1")
                            TextoB = TextoB + "N° Cuenta Soles: 191-2503044-0-72, Nombre: La Fiduciaria Fid. Silvestre/NeoAgrum-Silvestre.Rec. CCI: 002 191 00250304407258";
                        else
                            TextoB = TextoB + "N° Cuenta Dolar: 191-2496433-1-03, Nombre: La Fiduciaria Fid. Silvestre/NeoAgrum-Silvestre.Rec. CCI: 002 191 00249643310357";
                    }
                    if (IdEmpresa == 2)
                    {
                        if (objFacturaCabecera.TipoMoneda == "1")
                            TextoB = TextoB + "N° Cuenta Soles: 191-2503045-0-82, Nombre: La Fiduciaria Fid. Silvestre/NeoAgrum-NeoAgrum.Rec. CCI: 002 191 00250304508256";
                        else
                            TextoB = TextoB + "N° Cuenta Dolar: 191-2496434-1-13, Nombre: La Fiduciaria Fid. Silvestre/NeoAgrum-NeoAgrum.Rec. CCI: 002 191 00249643411355";
                    }
                    Texto.Texto3 = TextoB;
                }
                ...
                if ( CASO == "GINAFECTA" || CASO == "GGRAVADA" || CASO == "GEXONERADA")
                {
                    ...
                    objComprobante.ComprobanteMontosAdicionalesObligatorios = LlenarMontosAdicionalesObligatoriosFactura(objComprobante.ComprobanteDetalle.ToList(), intSolo).ToArray();
                ...
                if(objFacturaCabecera.NumeroRUC == "20191503482")
                {
                    if(objFacturaCabecera.ID_Documento == 700)
                    {
                        Multiglosa2.Add(objFacturaCabecera.Observaciones);
                        objComprobante.Multiglosa = Multiglosa2.ToArray();
                    }
                }
                else if (objFacturaCabecera.NumeroRUC == "20509089923")
                {
                    if (objFacturaCabecera.ID_Documento == 7001)
                    {
                        Multiglosa2.Add(objFacturaCabecera.Observaciones);
                        objComprobante.Multiglosa = Multiglosa2.ToArray();
                    }
                }
                else if (objFacturaCabecera.NumeroRUC == "20505467214")
                {
                        Multiglosa2.Add(objFacturaCabecera.Observaciones);
                        objComprobante.Multiglosa = Multiglosa2.ToArray();
                }
                ...
                FacturacionElectronicaOkWCF.ENDetraccion[] objDetraccion = new FacturacionElectronicaOkWCF.ENDetraccion[1];


                if (string.IsNullOrEmpty(NroCuenta))
                { throw new ArgumentException("ERROR: Ingresar Nro. Cuenta de BN"); }
                else
                {
                    ListaDetraccion = LlenarDetraccion(NroCuenta, lstDetalle, ref Tiene, objFacturaCabecera.TC);
                    if (Tiene > 0)
                    {
                        if (ListaDetraccion.Count > 0)
                        {
                            objDetraccion[0] = ListaDetraccion[0];
                            objComprobante.Detraccion = objDetraccion;
                        }
                    }
                }
        ...
        void LlenarComprobanteBoleta(int Op, string TablaOrigen, string serie, string numero, ref string Archivo)
        {
            ...
            try
            {
                ...
                //TREFI
                if (objBoletaCabecera.TREFI == 0)
                {
                    Texto.Texto3 = "Los derechos e importes contenidos en este documento han sido cedidos de manera irrevocable a un patrimonio autónomo administrado por Scotiabank Perú S.A.A.";
                }


                if (objBoletaCabecera.TREFI == 99)
                {
                    var IdEmpresa = ((Usuario_LoginResult)HttpContext.Current.Session["Usuario"]).idEmpresa;
                    var TextoB = "Los importes contenidos en el presente comprobante de pago han sido transferidos a un patrimonio fideicometido administrado por La Fiduciaria S.A. En consecuencia, los pagos debidos de acuerdo a este comprobante de pago deberán realizarse únicamente a la siguiente cuenta bancaria, abierta en: ";
                    ...
                    if (IdEmpresa == 1)
                    {
                        if (objBoletaCabecera.TipoMoneda == "1")
                            TextoB = TextoB + "N° Cuenta Soles: 191-2503044-0-72, Nombre: La Fiduciaria Fid. Silvestre/NeoAgrum-Silvestre.Rec. CCI: 002 191 00250304407258";
                        else
                            TextoB = TextoB + "N° Cuenta Dolar: 191-2496433-1-03, Nombre: La Fiduciaria Fid. Silvestre/NeoAgrum-Silvestre.Rec. CCI: 002 191 00249643310357";
                    }
        ...
        void LlenarComprobanteND(int Op, string TablaOrigen, string serie, string numero, ref string Archivo, string ReferenciaDocumentoTipo, string ReferenciaDocumentoNombre, string ReferenciaDocumentoSerie, string ReferenciaDocumentoNumero, string ReferenciaDocumentoFecha)
        {
            ...
            try
            {
                ...
                //objComprobante.dtComprobanteMotivoDocumentoSustento = dtTabla; 
                ...
                if (objNotaDebitoCabecera.TREFI == 0)
                {
                    Texto.Texto3 = "Los derechos e importes contenidos en este documento han sido cedidos de manera irrevocable a un patrimonio autónomo administrado por Scotiabank Perú S.A.A.";
                }

                if (objNotaDebitoCabecera.TREFI == 99)
                {
                    var IdEmpresa = ((Usuario_LoginResult)HttpContext.Current.Session["Usuario"]).idEmpresa;
                    var TextoB = "Los importes contenidos en el presente comprobante de pago han sido transferidos a un patrimonio fideicometido administrado por La Fiduciaria S.A. En consecuencia, los pagos debidos de acuerdo a este comprobante de pago deberán realizarse únicamente a la siguiente cuenta bancaria, abierta en: ";
                    ...
                    if (IdEmpresa == 1)
                    {
                        if (objNotaDebitoCabecera.TipoMoneda == "1")
                            TextoB = TextoB + "N° Cuenta Soles: 191-2503044-0-72, Nombre: La Fiduciaria Fid. Silvestre/NeoAgrum-Silvestre.Rec. CCI: 002 191 00250304407258";
                        else
                            TextoB = TextoB + "N° Cuenta Dolar: 191-2496433-1-03, Nombre: La Fiduciaria Fid. Silvestre/NeoAgrum-Silvestre.Rec. CCI: 002 191 00249643310357";
                    }
        ...
        List<FacturacionElectronicaOkWCF.ENComprobanteDetalle> LlenarDetallesFac(int Op, string TablaOrigen, string serie, ref string Archivo, string CASO, List<VBG04708_DETALLEResult> lstDetalle)
        {
            ...
            try
            {
                ...
                foreach (VBG04708_DETALLEResult objFacturaDetalle in lstDetalle)
                {
                    ...
                    if (objENDetalle.Codigo == "S5693" || objENDetalle.Codigo == "s2725" || objENDetalle.Codigo == "S7845")
                    {
                        objENDetalle.Total =  ((Math.Abs(objENDetalle.Total)));
                    }
        ...
        List<FacturacionElectronicaOkWCF.ENComprobanteMontosAdicionalesObligatorios> LlenarMontosAdicionalesObligatoriosFactura(List<FacturacionElectronicaOkWCF.ENComprobanteDetalle> ListaDetalle, int intSolo )
        {
                ...
                if (x == 0)
                {
                    ...
                    foreach (FacturacionElectronicaOkWCF.ENComprobanteDetalle Item in ListaDetalle)
                    {
                        if (Item.ComprobanteDetalleImpuestos[0].AfectacionIGV == "10")
                        {
                            ...
                            {
                                if (Item.Codigo == "S5693" || Item.Codigo == "s2725" || Item.Codigo == "S7845")
                                {
                                    objMontosObligatorios.Monto = objMontosObligatorios.Monto + (( Math.Abs(Item.Total)) * -1);
                                }
                                else
                                {
                                    objMontosObligatorios.Monto = objMontosObligatorios.Monto + Item.Total;
        ...
        List<FacturacionElectronicaOkWCF.ENDetraccion> LlenarDetraccion(string NroCuenta, List<VBG04708_DETALLEResult> lstDetalle, ref int Tiene, decimal TC)
        {
            List<FacturacionElectronicaOkWCF.ENDetraccion> ListaDetraccion = new List<FacturacionElectronicaOkWCF.ENDetraccion>();
            ...
            List<VBG02714_DetraccionResult> listaDetraccion = new List<VBG02714_DetraccionResult>();
            VBG02714_DetraccionResult Detraccion = new VBG02714_DetraccionResult();
            FacturaElectronica2WCFClient objFacturaWCF = new FacturaElectronica2WCFClient();

            decimal ImporteTotalSoles;

            if(objComprobante.Moneda == "USD")
            {
                ImporteTotalSoles = (objComprobante.ImporteTotal * TC);
            }
            else
            {
                ImporteTotalSoles = (objComprobante.ImporteTotal); 
            }


                foreach (VBG04708_DETALLEResult obj in lstDetalle)
                {
                    listaDetraccion = objFacturaWCF.Detranccion_Item(((Usuario_LoginResult)Session["Usuario"]).idEmpresa, ((Usuario_LoginResult)Session["Usuario"]).codigoUsuario, Convert.ToInt32(obj.Kardex), 14).ToList();
                }
            Tiene = 0;
            if (listaDetraccion.Count > 0)
                {
                    Detraccion = listaDetraccion[0];

                        if (ImporteTotalSoles > Detraccion.ImporteMinimoAfecto)
                        {
                                ...
                                objNumeroCuenta.Valor = NroCuenta;
                                ...
                                objPorcentaje.Valor = Detraccion.Porcentaje;
        ...
        List<FacturacionElectronicaOkWCF.ENComprobanteMotivoDocumento> LlenarMotivoDocumento(string CodigoMotivoEmision,  string ReferenciaDocumentoSerie, string ReferenciaDocumentoNumero, string Sustento)
        {
